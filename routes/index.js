...
router.get('/', function(req, res, next) {
  ...
  Promise.all([
    db.query('SELECT portfolio.*, project_types.singular_name as type_name FROM portfolio LEFT JOIN project_types ON portfolio.type_id = project_types.id WHERE (type_id = 2 OR type_id = 3) AND status = $<status> ORDER BY portfolio_date DESC LIMIT 1', {
      status: 'published'
    }),
    db.query('SELECT portfolio.*, project_types.singular_name as type_name FROM portfolio LEFT JOIN project_types ON portfolio.type_id = project_types.id WHERE type_id = 1 AND status = $<status> ORDER BY portfolio_date DESC LIMIT 1', {
      status: 'published'
    }),
  ])
  .then(function([
    ...
    const projects = [...appsOrGames, ...sites].filter(projectFromDb => !!projectFromDb).map(projectFromDb => new PortfolioProject(projectFromDb, getUserLanguage(req).code_name));

...
router.get('/:lang/', addTrailingSlash, function(req, res, next) {
...
function processHomePage(req, res, next) {
  ...
  Promise.all([
    db.query('SELECT portfolio.*, project_types.singular_name as type_name, project_types.plural_name as type_plural_name FROM portfolio LEFT JOIN project_types ON portfolio.type_id = project_types.id WHERE type_id = 3 AND status = $<status> ORDER BY portfolio_date DESC LIMIT 5', {
      status: 'published'
    }),
  ])
  .then(function([
    ...
    const projects = [...games].filter(projectFromDb => !!projectFromDb).map(projectFromDb => new PortfolioProject(projectFromDb, getUserLanguage(req).code_name));
